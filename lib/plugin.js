...
function Plugin ( config ) {
    ...
    this.profiles.forEach(function ( profile ) {
        var buildTask = profile.task(self.entry, function ( done ) {
            ...
            buildWatcher, cacheWatcher;

        if ( profile.data.watch && profile.data.watch.build ) {
            // rebuild on files change
            profile.task('build:watch', function ( done ) {
                buildWatcher = self.watch(profile.data.watch.build, buildTask);
                buildWatcher.done = done;
            });

            // stop watching
            profile.task('build:unwatch', function () {
                if ( buildWatcher ) {
                    // finish chokidar
                    buildWatcher.close();
