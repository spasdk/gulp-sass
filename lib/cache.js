...
module.exports = function ( profile ) {
    ...
    normalize(JSON.parse(fs.readFileSync(profile.jsCache))).forEach(function ( item ) {
        var varsFile = path.join('..', '..', '..', item, profile.varsFile),
            mainFile = path.join('..', '..', '..', item, profile.mainFile);

        if ( fs.existsSync(path.join(item, profile.varsFile)) ) {
            srcVars.push(varsFile);
        } else {
            srcVars.push('missing:' + varsFile);
        }

        if ( fs.existsSync(path.join(item, profile.mainFile)) ) {
            srcMain.push(mainFile);
        } else {
            srcMain.push('missing:' + mainFile);
        }
    });
    ...
    srcVars.push(path.join('..', profile.varsFile));
    srcMain.push(path.join('..', profile.mainFile));
